#!/usr/bin/env python3
"""
Upload a video to YouTube via the YouTube Data API v3 with OAuth credentials.

Usage:
    python youtube-upload.py --metadata metadata.json --video video.mp4 [--thumbnail thumb.jpg] [--visibility public]

Metadata JSON should contain: title, description, tags, category, visibility/privacy.
...
def load_credentials():
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            print(
                f"  python {os.path.dirname(os.path.abspath(__file__))}/youtube-reauth.py",
                file=sys.stderr,
            )
            sys.exit(1)
...
def upload_video(youtube, video_path, meta, visibility):
...
def main():
    ...
    parser.add_argument(
        "--metadata", required=True, help="Path to metadata JSON file"
    )
    parser.add_argument("--video", required=True, help="Path to video file")
    parser.add_argument("--thumbnail", help="Path to thumbnail image (overrides metadata)")
    parser.add_argument(
        "--visibility",
        choices=["public", "unlisted", "private"],
        help="Override visibility from metadata",
    )
    ...
    # Resolve visibility: CLI flag > metadata field
    visibility = (
        args.visibility
    ...
    # Resolve thumbnail: CLI flag > metadata field
    ...
    # Upload
    response = upload_video(youtube, args.video, meta, visibility)
    ...
    # Print JSON result to stdout for agent parsing
    result = {
        "video_id": video_id,
        "url": video_url,
        "title": meta["title"],
