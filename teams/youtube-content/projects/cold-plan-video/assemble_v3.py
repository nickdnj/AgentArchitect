#!/usr/bin/env python3
"""
Cold Plan Video Assembly v3
Adds cold-open scene (scene_00), updates title card duration to 3s,
swaps in v2 narration for scenes 02 and 13, reuses all other segments.
Rebuilds audio track from scratch (narration positions shift due to new scene).
"""
...
# ------------------------------------------------------------------
# Scene definitions — NEW 24-scene order for v3
#
# Format: (seg_id, image_stem, audio_stem_or_None, duration_s, style,
#          overlay_text, overlay_config)
#
# Duration = narration duration + ~0.5s buffer (or fixed for non-narrated)
# seg_id is the output file name (seg_00.mp4 through seg_23v.mp4)
# ------------------------------------------------------------------
...
SCENES = [
    ...
    ("07",    "scene_07",           "scene_07",             23.6,  "gentle-zoom",    "NyQuil = 3 pills\n$12  vs  $0.12",                "lower_third"),
    ("08",    "scene_08",           "scene_08",             20.9,  "gentle-zoom",    "DayQuil = 3 pills\n$12  vs  $0.19",               "lower_third"),
    ("09",    "scene_09",           "scene_09",             19.2,  "gentle-zoom",    "Advil Cold & Sinus = 2 pills\n$10  vs  $0.12",    "lower_third"),
    ("10",    "scene_10",           "scene_10",             24.3,  "ken-burns-zoom", "Pseudoephedrine: Behind the counter.\nNo prescription needed.", "lower_third"),
    ("11",    "scene_11",           "scene_11",             20.8,  "gentle-zoom",    "Theraflu = 3 pills + hot water\n$10  vs  $0.23",  "lower_third"),
    ("12",    "scene_12",           "scene_12",             15.7,  "gentle-zoom",    "Advil PM = 2 pills\n$14  vs  $0.05",              "lower_third"),
...
# Segments that must be (re)generated — all others are reused from v2
REGENERATE = {"00", "01", "02", "13"}
...
def make_segment(seg_id, img_stem, duration, style, text, text_config):
    ...
    run(cmd, f"Segment {seg_id} {style}")


# ------------------------------------------------------------------
# STEP 1: Generate only the changed/new segments
# ------------------------------------------------------------------
print("\n" + "="*60)
print("STEP 1: Generating new/changed segments")
...
concat_file = f"{ASSEMBLY}/concat_v3.txt"
with open(concat_file, "w") as cf:
    for (seg_id, *_) in SCENES:
        cf.write(f"file '{SEGMENTS}/seg_{seg_id}.mp4'\n")

video_only = f"{ASSEMBLY}/video-only-v3.mp4"
run([
    "ffmpeg", "-y",
    "-f", "concat", "-safe", "0", "-i", concat_file,
    "-c", "copy",
    video_only
], "Concat video-only-v3")
...
narr_norm = {}  # audio_stem -> normalized wav path
...
narr_continuous = f"{ASSEMBLY}/narr-continuous-v3.wav"
run(
    ["ffmpeg", "-y"] + inputs + [
...
if HAS_SF:
    ...
    music_wav = f"{ASSEMBLY}/music_v3.wav"
    ...
    # (both should be 44100 after normalization — skip if matched)
    if sr_m != sr:
        print(f"WARNING: Music SR {sr_m} != narration SR {sr}. Resampling via FFmpeg.")
        music_resampled = f"{ASSEMBLY}/music_resampled.wav"
    ...
    final_audio = f"{ASSEMBLY}/final-audio-v3.wav"
...
final_out = f"{OUTPUT}/cold-plan-final-v2.mp4"
run([
    "ffmpeg", "-y",
    "-i", video_only,
    "-i", final_audio,
    ...
    "-shortest",
    final_out
], "Final encode cold-plan-final-v2.mp4")
...
# Confirm cold open is at position 0
print(f"\nCold open segment: {SEGMENTS}/seg_00.mp4")
seg00_probe = subprocess.run([
    "ffprobe", "-v", "error",
    "-show_entries", "format=duration",
    "-of", "default=noprint_wrappers=1:nokey=1",
    f"{SEGMENTS}/seg_00.mp4"
], capture_output=True, text=True)
print(f"Cold open duration: {float(seg00_probe.stdout.strip()):.2f}s")

print("\nASSEMBLY V3 COMPLETE")
