...
<!-- collaboration: {"can_request_from":["chrome-browser"],"provides_to":["all"],"handoff_format":"structured-summary","primary_collaborators":["archivist","email-research","proposal-review"]} -->
<!-- expertise: {"domains":["document-processing","ocr","content-extraction","transcription","rag","semantic-search"],"capabilities":["PDF text extraction","Image-based PDF transcription (Claude Sonnet 4 or GPT-4o vision)","Visual content description","Table to HTML conversion","Multi-source PDF retrieval","Intelligent caching with checksum validation","Local file system access (including Google Drive synced folders)","RAG ingestion (chunk + embed + store in pgvector)","Semantic search across indexed documents","Multi-provider AI support (Anthropic/OpenAI)"]} -->
...
**RAG CLI Commands:**
- **ingest:** `python src/rag.py ingest <file> --bucket <bucket_id>`
- **search:** `python src/rag.py search '<query>' --bucket <bucket_id>`
- **list:** `python src/rag.py list --bucket <bucket_id>`
- **stats:** `python src/rag.py stats`

**Always run RAG search before Glob/Grep for policy queries.**

### Storage Paths

- **Output folder:** `outputs/`
- **CLI tool path:** `${PDFSCRIBE_CLI_PATH}`

### CLI Tool Commands

**Working directory:** `${PDFSCRIBE_CLI_PATH}`

- **Transcribe:** `python pdfscribe_cli.py`
- **PDF to Website:** `python pdf2website.py`
- **Split PDF:** `python src/split_pdf.py`
- **RAG operations:** `python src/rag.py`

**Requires one of:** ANTHROPIC_API_KEY or OPENAI_API_KEY
**RAG requires:** OPENAI_API_KEY, RAG_DB_HOST or localhost:5433
