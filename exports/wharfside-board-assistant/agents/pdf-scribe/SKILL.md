...
PDFScribe leverages the **pdfscribe_cli** tool located at `${PDFSCRIBE_CLI_PATH}/`.
...
```bash
python pdfscribe_cli.py ${GOOGLE_DRIVE_PATH} drives/path/to/document.pdf
...
- **Local / Google Drive synced**: Use directly (files in `${GOOGLE_DRIVE_PATH} are local)
...
```bash
python pdfscribe_cli.py ${GOOGLE_DRIVE_PATH} drives/Governing\ Documents/document.pdf
```
This will:
...
**Examples:**
- `Source: local, Location: ${GOOGLE_DRIVE_PATH} drives/Governing Documents/document.pdf`
...
- Local file system for reading/writing (including Google Drive synced folders at `${GOOGLE_DRIVE_PATH}
...
- Location: ${GOOGLE_DRIVE_PATH} drives/Infrastructure/
- Pages: 12
...
- Check that the target folder exists in `${GOOGLE_DRIVE_PATH}
...
2. **Use full paths for Google Drive folders** - e.g., `${GOOGLE_DRIVE_PATH} drives/...`
